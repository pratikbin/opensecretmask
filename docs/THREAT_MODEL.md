...
### 3.8 Upstream TLS trust includes the osm CA

The proxy's upstream `http.Transport` verifies LLM endpoint certificates
against the system root pool **plus the local osm CA**. This lets the
proxy accept upstream responses signed by its own CA (used by integration
and e2e test mocks) without exposing a CLI flag, and lets the same CA
serve corporate LLM gateways behind a private root.
...
A caller that explicitly passes `proxy.Config{UpstreamTLS: ...}` (e.g. a
custom embedding) is unaffected: their config is used verbatim and the
osm CA is **not** auto-added.

### 3.9 Rogue tools and MCP servers
