...
| Host | Exact-host match against `proxy.DefaultProviders` + `--provider` entries. Non-matching CONNECT is tunneled byte-for-byte. Cloud platforms with per-resource or per-region hostnames — Azure OpenAI, AWS Bedrock, watsonx, Databricks, OCI — are **not** in the default list and route through unmasked unless you add them with `--provider <host>`. | `internal/proxy/proxy.go` OnRequest HandleConnectFunc, DefaultProviders doc comment; `README.md` |
...
| Headers | Never masked. `Authorization` / `x-api-key` must reach the provider with the real credential. Secrets in headers for any other purpose are not protected. `Content-Length` is deleted unconditionally on every in-scope request with a body that reaches masking, regardless of whether a secret matched; no other header is touched. Byte-for-byte forwarding of the rest of the header set is not separately tested beyond the proxy round-trip suite. | `internal/proxy/proxy.go` onRequest |
