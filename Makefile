.PHONY: build test test-integration test-e2e e2e-bins lint vuln fix all clean
...
# test-e2e builds the e2e binaries and runs the suite. Requires Docker and
# ANTHROPIC_AUTH_TOKEN to actually exercise the claude-code paths; the
# mockupstream-driven TestE2E_Run_MaskRoundTrip needs Docker only.
test-e2e: e2e-bins
